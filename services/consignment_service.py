...
def validate_and_normalize_rows(rows, deleted_ids, normalize_consignment_number, normalize_status, normalize_indian_pincode):
    ...
    for raw_deleted_id in deleted_ids:
        try:
            ...
            return None, None, [{"message": f"Invalid deleted row id: {raw_deleted_id}"}]
...
def apply_consignment_changes(validated_rows, validated_deleted_ids, Consignment, db, store_pod_bytes_func, delete_pod_file_func):
    ...
    with transaction(db):
        ...
        for row in validated_rows:
            ...
            if row.get("pod_file_data"):
                ...
                new_pod_image = store_pod_bytes_func(filename, pod_bytes, row.get("pod_file_type"))
