...
def import_from_workbook(file_obj, Consignment=None, db=None, normalize_consignment_number=None, normalize_status=None, normalize_indian_pincode=None):
    ...
    if normalize_consignment_number is None or normalize_status is None or normalize_indian_pincode is None:
    ...
    if Consignment is not None:
        try:
            existing_numbers = {c[0] for c in Consignment.query.with_entities(Consignment.consignment_number).all()}
        except Exception:
