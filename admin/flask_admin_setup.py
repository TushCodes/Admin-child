...
from .controllers import build_backup_payload
from .controllers.responses import json_error, json_success
from .db.session import transaction
from .models import Consignment, Lead, db
from .services.consignment_importer import (
...
from .services.pdf_export import generate_consignment_pdf
from .services.pod_storage import (
    delete_pod_file,
...
class ConsignmentAdminView(SecureModelView):
    ...
    @expose("/<int:consignment_id>/pod", methods=["GET"])
    def consignment_pod_file(self, consignment_id):
        try:
    ...
    @expose("/<int:consignment_id>/pod", methods=["POST"])
    def consignment_pod_upload(self, consignment_id):
        upload = request.files.get("file")
        if not upload or not upload.filename:
    ...
    @expose("/<int:consignment_id>/pod", methods=["DELETE"])
    def consignment_pod_delete(self, consignment_id):
        try:
            consignment = db.session.get(Consignment, consignment_id)
            if not consignment or not getattr(consignment, "pod_image", None):
